export default function FinancialCardShell({
  ...
  return (
    <article className="group relative flex h-full max-h-full min-h-0 w-full flex-col overflow-hidden rounded-[26px] border border-white/[0.12] bg-[#0b1118]/82 p-4 text-white shadow-[0_22px_55px_rgba(0,0,0,0.34)] backdrop-blur-2xl transition duration-300 active:scale-[0.992]">
      ...
      <div className="relative z-10 flex h-full min-h-0 flex-1 flex-col">
        ...
        <section className="mt-4 shrink-0">
          ...
          {heroSubtext && (
            <p className="mt-2 truncate text-[12px] font-medium text-white/45">
              {heroSubtext}
            </p>
          )}
        </section>
        ...
        <section className="mt-4 min-h-0 flex-1">
          ...
          {insight && (
            <p className="mt-3 line-clamp-2 text-[12px] leading-relaxed text-white/58">
              {insight}
            </p>
          )}
        </section>

        {children && <div className="relative z-10 mt-auto shrink-0 pt-4">{children}</div>}
      </div>
    </article>
  );
}
